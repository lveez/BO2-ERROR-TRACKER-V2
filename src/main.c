...
int main(int argc, char** argv) {
    ...
    ushort num_ents_free = 0;
    uint first_free_ent = 0;
    ...
    while (1) {
        ...
        process_id = GetProcessID("t6zmv41.exe");
        if (!process_id) {
            process_id = GetProcessID("plutonium-bootstrapper-win32.exe");
        }
        ...
        while (1) {
            ...
            if (options.gspawn) {
                num_ents_in_use = 0;
                error = ReadValue(process_handle, gLevel.gentitites, &gEntities, sizeof(gentity_t) * 1022);
                if (error) {
                ...
                error = ReadValue(process_handle, 0x2338790, &first_free_ent, sizeof(first_free_ent));
                if (error) {
                    printf("ERROR4: ReadValue failed (code %d).\n", error);
                    CloseHandle(process_handle);
                    break;
